...
async def process_audio_image_pipeline(audio_file: UploadFile, image_files: List[UploadFile], db: Session):
    if not audio_file:
    ...
    return upload, str(audio_path), saved_images


def run_pipeline_background(upload_id: str, audio_path: str, image_paths: List[str]):
    db = SessionLocal()
    try:
        upload = db.query(Upload).filter(Upload.id == upload_id).first()
        ...
        analysis = _summarize_images(transcript, image_paths)
